...
using Northwind.Core.Domain.Common;
...
namespace Northwind.Core.Infra.Repositories
{
    public class RepositoryGeneric<TEntity> : IRepositoryGeneric<TEntity> where TEntity : class
    {
        ...
        public RepositoryGeneric(NorthwindContext context)
        {
            _context = context;
            _dbSet = _context.Set<TEntity>();
        }

        public IQueryable<TEntity> FromSql(string query, object[] parameters) => _dbSet.FromSql(query, parameters);
        ...
        public async Task<TEntity> FindAsync(IncludeList<TEntity> includes, Expression<Func<TEntity, bool>> filter)
        {
            ...
            if (count > 1)
                throw new Exception("More than one entity find for the specified filter");
